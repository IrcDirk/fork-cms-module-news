...
class Category extends FrontendBaseBlock
{
    ...
    {
        // create the form
        $this->frm = new FrontendForm('newsIndexForm', null, 'get', null, false);
        $categories = FrontendNewsCategoriesModel::getForMultiCheckboxForParent($this->record['id']);
    ...
    private function getData()
    {
        ...
        if (empty($parameter)) {
         ...
         $this->record = FrontendNewsCategoriesModel::get($parameter);
        ...
        if($this->filter['form'] == NULL) {
            $children = FrontendNewsCategoriesModel::getAllChildrenByPath( $this->record['path']);
        ...
        // set URL and limit
        $this->pagination['url'] = Navigation::getURLForBlock('News') . '?' . http_build_query($this->filter);
        $this->pagination['limit'] = $this->get('fork.settings')->get('News', 'overview_num_items', 10);
        $this->pagination['limit'] = 100;

        // populate count fields in pagination
        $this->pagination['num_items'] = FrontendNewsModel::getAllCount($this->filter);
        ...
        // get articles
        $this->items = FrontendNewsModel::getAll($this->pagination['limit'], $this->pagination['offset'], $this->filter);
    }
    ...
    private function parse()
    {
        ...
        $allCategories = FrontendNewsCategoriesModel::getAll(array('parent_id' => 0));
        unset($allCategories[$this->record['id']]);
        $this->tpl->assign('widgetNewsCategories',$allCategories  );
