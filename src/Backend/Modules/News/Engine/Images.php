...
class Images
{
    const QRY_DATAGRID_BROWSE_IMAGES_FOR_PROJECT  =
        ...
        public static function getURL($URL, $language, $id = null)
        {
            ...
            if ($id === null) {
                ...
                if ((bool) $db->getVar(
                    'SELECT 1
                     FROM news AS i
                     INNER JOIN news_post_images_content AS m ON i.id = m.image_id
        ...
        */
        ...
        {
            BackendModel::get('database')->delete('news_post_images', 'id = ?', (int) $id);
            BackendModel::get('database')->delete('news_post_images_content', 'image_id = ?', (int) $id);
        }
        ...
        */
        public static function get($id)
        {
           ...
           $return['content'] = (array) $db->getRecords(
               'SELECT i.* FROM news_post_images_content AS i
               WHERE i.image_id = ?',
    ...
    public static function getAll($id)
    {
        ...
        $return =  (array) $db->getRecords(
               'SELECT i.*
                FROM news_post_images AS i
                WHERE i.news_post_id = ? ORDER BY i.sequence',
               array((int) $id)
           );
        ...
        */
        public static function insert(array $item)
        {
            ...
            return (int) BackendModel::get('database')->insert('news_post_images', $item);
        }

    public static function insertContent(array $content)
    {
        BackendModel::get('database')->insert('news_post_images_content', $content);
    }
        ...
        */
        public static function update(array $item)
        {
            ...
            BackendModel::get('database')->update(
               'news_post_images', $item, 'id = ?', (int) $item['id']
           );
        }
    ...
    public static function updateContent(array $content, $id)
    {
        ...
        foreach ($content as $language => $row) {
            $db->update('news_post_images_content', $row, 'image_id = ? AND language = ?', array($id, $language));
        }
    }
