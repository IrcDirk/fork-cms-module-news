...
class EditCategory extends ActionEdit
{
    ...
    protected function loadData()
    {
        ...
        if ($this->id == null || !BackendNewsCategoryModel::exists($this->id)) {
            $this->redirect(
                Model::createURLForAction('Index') . '&error=non-existing'
            );
        }

        $this->record = BackendNewsCategoryModel::get($this->id);
    }
    ...
    protected function loadForm()
    {
        ...
        $categories = BackendNewsCategoryModel::getForDropdown();
        ...
        if ($this->frm->isSubmitted()) {
            ...
            if ($this->frm->isCorrect()) {
                ...
                if ($item['parent_id'] == 0) {
                    ...
                    $category = BackendNewsCategoryModel::get($item['parent_id']);
                ...
                foreach ($this->languages as $language) {
                    ...
                    $specific['url'] =  BackendNewsCategoryModel::getURL(CommonUri::getUrl($specific['name']), $language['abbreviation'], $item['id']);
                ...
                BackendNewsCategoryModel::update($item);
                BackendNewsCategoryModel::updateContent($content, $item['id']);
